...
if __name__ == "__main__":
    ...
    if generated_pil_image is None:
        ...
        # ▼▼▼▼▼【ロジック変更箇所】▼▼▼▼▼
        # --- 2-6. 反復的なピーク検出とマスク処理 ---
        print(f"\n🔍 反復的なピーク検出を開始 (最大{MAX_PEAKS_TO_FIND}個、しきい値 > {PEAK_SALIENCY_THRESHOLD})")
        ...
        for i in range(MAX_PEAKS_TO_FIND):
            ...
            if val_n < PEAK_SALIENCY_THRESHOLD:
            ...
            # 4. ピークNの周辺をマスクするための半径を計算
            mask_radius = MIN_MASK_RADIUS # デフォルトは最小半径
            if y_low.size > 0:
                # しきい値を下回る全ピクセルとピークN (x_n, y_n) との距離を計算
            ...
            # 5. temp_map 上のピークNの周辺をマスク（消去）する
